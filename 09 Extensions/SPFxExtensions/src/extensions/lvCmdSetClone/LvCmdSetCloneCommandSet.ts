...
import * as strings from "LvCmdSetCloneCommandSetStrings";
 ...
 */
export interface ILvCmdSetCloneCommandSetProperties {}

const LOG_SOURCE: string = "LvCmdSetCloneCommandSet";

export default class LvCmdSetCloneCommandSet extends BaseListViewCommandSet<
  ILvCmdSetCloneCommandSetProperties
> {
  ...
  public onInit(): Promise<void> {
    Log.info(LOG_SOURCE, "Initialized LvCmdSetCloneCommandSet");
    pnp.setup({
    ...
    switch (event.itemId) {
      case "spfxClone":
        this.ensureListSchema() //Go get the field information
          .then(
            (listFields: Array<IListField>): void => {
                      ...
                      switch (field.TypeAsString) {
                        ...
                        case "LookupMulti":
                          ...
                          result[field.InternalName].forEach((prop: any) => {
                            item[field.InternalName + "Id"].results.push(
                              prop["Id"]
                            );
                          });
                          break;
  ...
  private ensureListSchema(): Promise<Array<IListField>> {
    return new Promise<Array<IListField>>(
      (
        resolve: (listFields: Array<IListField>) => void,
        ...
        if (this._listFields) {
          ...
          pnp.sp.web.lists
            .getById(this.context.pageContext.list.id.toString())
            .fields.select("InternalName", "TypeAsString", "IsDependentLookup")
            .getAs<IListField[]>()
            .then((results: IListField[]) => {
              ...
              for (let field of results) {
                if (
                  this._fieldTypesToIgnore.indexOf(field.TypeAsString) == -1 &&
                  this._fieldsToIgnore.indexOf(field.InternalName) == -1 &&
                  !field.IsDependentLookup
                ) {
