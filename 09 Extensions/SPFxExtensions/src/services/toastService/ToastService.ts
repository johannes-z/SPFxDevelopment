...
export class ToastService {
  ...
  public static getToastsFromList(
    spHttpClient: SPHttpClient,
    baseUrl: string
  ): Promise<IToast[]> {
    return spHttpClient
      .get(
        `${baseUrl}/${ToastService.apiEndPoint}?$select=${
          ToastService.select
        }&$orderby=${ToastService.orderby}`,
        SPHttpClient.configurations.v1
      )
      .then(
        (response: SPHttpClientResponse): Promise<{ value: IToast[] }> => {
          if (!response.ok) {
            //Failed requests don't automatically throw exceptions which
            // can be problematic for chained promises, so we throw one
            throw `Unable to get items: ${response.status} (${
              response.statusText
            })`;
          }
